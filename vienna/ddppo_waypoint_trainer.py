...
from vienna.common.ddppo_alg import WDDPPO, CWDDPPO, CMADDPPO, ILP, MTDDPPO
from vienna.common.env_utils import (
    construct_envs,
    construct_envs_auto_reset_false,
)
from vienna.common.rollout_storage import ActionDictRolloutStorage, CMARolloutStorage, TFRolloutStorage
from vienna.common.utils import extract_instruction_tokens
from vienna.config.default import add_pano_sensors_to_config

task_name = ['vln','van','ign','ogn']
...
class DDPPOWaypointTrainer(PPOTrainer):
    ...
    def _initialize_policy(
        ...
        # if config.RL.DDPPO.pretrained:
            ...
            extra_state = ckpt_dict['extra_state']
            requeue_stats = extra_state["requeue_stats"]
    ...
    def train(self) -> None:
        ...
        print('world rank', self.world_rank, 'world size', self.world_size)
        ...
        if interrupted_state is not None:
            ...
            print('keys', interrupted_state.keys())
            self.agent.load_state_dict(interrupted_state["state_dict"], strict=False)
            extra_state = interrupted_state['extra_state']
            # self.agent.optimizer.load_state_dict(
            #     extra_state["optim_state"]
            # )
            # lr_scheduler.load_state_dict(extra_state["lr_sched_state"])
            # print('optim', extra_state["optim_state"])
            # print('lr sched', extra_state["lr_sched_state"])
            ...
            for update in range(start_update, self.config.RL.NUM_UPDATES):
                ...
                if len(window_logging_predictions):
                    ...
                    for k, v in preds.items():
                        writer.add_scalar(f"preds/{k}", v, count_steps)
                ...
                for k, v in losses.items():
                    writer.add_scalar(f"losses/{k}", v, count_steps)
                ...
                for k, v in entropies.items():
                    writer.add_scalar(f"entropies/{k}", v, count_steps)
    ...
    #     ckpt_path = self.config.EVAL_CKPT_PATH_DIR
...
@baseline_registry.register_trainer(name="ddppo-waypoint-tf")
class DDPPOWaypointTrainerTF(DDPPOWaypointTrainer):
    def _set_observation_space(self, envs, batch, instruction_uuid):
        obs_space = apply_obs_transforms_obs_space(
            envs.observation_spaces[0], self.obs_transforms
        )
        if instruction_uuid in batch:
            obs_space.spaces[instruction_uuid] = spaces.Box(
                low=np.finfo(np.float).min,
                high=np.finfo(np.float).max,
                shape=tuple(batch[instruction_uuid].shape[1:]),
        ...
        if 'rgb_features' in batch:
            obs_space.spaces['rgb_features'] = spaces.Box(
                low=np.finfo(np.float).min,
                high=np.finfo(np.float).max,
                shape=tuple(batch['rgb_features'].shape[1:]),
                dtype=np.float,
            )

        if 'depth_features' in batch:
            obs_space.spaces['depth_features'] = spaces.Box(
                low=np.finfo(np.float).min,
                high=np.finfo(np.float).max,
                shape=tuple(batch['depth_features'].shape[1:]),                dtype=np.float,
            )
        
        self.obs_space = obs_space

    def extract_visual_feature(self, obs):
        rgb = obs['rgb']
        depth = obs['depth']
        rgb_shape = rgb.shape
        ...
        B, M, H, W, C = rgb_shape
        rgb_embedding_batch = []
        depth_embedding_batch = []
        for i in range(B):
            rgb_ = rgb[i].reshape(M, *rgb_shape[2:]).permute(0, 3, 1, 2).to(self.device)
            depth_ = depth[i].reshape(M, *depth_shape[2:]).to(self.device)
            depth_obs = {'depth': depth_}
            
            with torch.no_grad():
                rgb_embedding = self.policy.net.rgb_encoder.extract_img(rgb_)
                depth_embedding = self.policy.net.depth_encoder.visual_encoder(depth_obs)
            
            rgb_embedding_batch.append(rgb_embedding)
            depth_embedding_batch.append(depth_embedding)

        rgb_embedding = torch.stack(rgb_embedding_batch, 0)
        depth_embedding = torch.stack(depth_embedding_batch, 0)

        # rgb_embedding_shape = rgb_embedding.shape
        # depth_embedding_shape = depth_embedding.shape

        # rgb_embedding = rgb_embedding.reshape(B, M, *rgb_embedding_shape[1:])
        # depth_embedding = depth_embedding.reshape(B, M, *depth_embedding_shape[1:])
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            step_observation = {
                k: v[rollouts.step].unsqueeze(0).to(self.device) for k, v in rollouts.observations.items()
            }
            # for k, v in step_observation.items():
            #     print(k, v.shape)

            step_observation['pre_e_t'] = rollouts.pre_e_t[:rollouts.step].to(self.device)
            step_observation['pre_q_t'] = rollouts.pre_q_t[:rollouts.step].to(self.device)

            step_prev_actions = {
                k: v[rollouts.step].to(self.device) for k, v in rollouts.prev_actions.items()
            }
            ...
            if np.random.rand() < 0:
                actions_componets = step_observation['vln_oracle_waypoint_sensor']
                action_elements = {}
                action_elements['pano'] = actions_componets[..., 0].squeeze(0).unsqueeze(-1).long()
                action_elements['offset'] = actions_componets[..., 1].squeeze(0).unsqueeze(-1)
                action_elements['distance'] = actions_componets[..., 2].squeeze(0).unsqueeze(-1)

                # print('action shape', actions_componets.shape)
                
                thetas = actions_componets[..., 3].squeeze(0)
                actions = []
                for i, theta in enumerate(thetas):
                    pano = action_elements['pano'][i]
                    if pano == self.config.TASK_CONFIG.TASK.PANO_ROTATIONS:
                        actions.append({"action": "STOP"})
                    else:
                        r = action_elements['distance'][i].item()
                        actions.append({
                            "action": {
                ...
                with torch.no_grad():
                    _, action_log_probs, _, _ = self.policy.evaluate_actions(step_observation,
                        step_prev_actions,
                        rollouts.masks[rollouts.step],
                        action_elements
                        )
                # print('probs', action_log_probs)

        # print('hshape',recurrent_hidden_states.shape)
        # print('actions', actions)
        # print('action_elements', action_elements)

        pth_time += time.time() - t_sample_action
        t_step_env = time.time()

        # obs_history = {
        #     "rgb": torch.zeros_like(step_observation["rgb"][0, :, 0]),
        #     "depth": torch.zeros_like(step_observation["depth"][0, :, 0]),
        # }
        ...
        for i in range(self.envs.num_envs):
            ...
            if actions[i]["action"] != "STOP":
                idx = action_elements["pano"][i]
                # obs_history["rgb"][i] = step_observation["rgb"][0, i, idx]
                # obs_history["depth"][i] = step_observation["depth"][0, i, idx]
                logging_predictions["distance_pred"].append(
                    self.policy.net.distance_to_continuous(
                        action_elements["distance"][i]
                    ).item()
        ...
        if rollouts.step == ppo_cfg.num_steps - 1:
        ...
        rgb_embedding, depth_embedding = self.extract_visual_feature(batch)

        del batch['rgb']
        del batch['depth']
        batch['rgb_features'] = rgb_embedding
        batch['depth_features'] = depth_embedding
        ...
        current_episode_reward += rewards
        running_episode_stats["reward"] += (1 - ended) * (1 - masks) * current_episode_reward
        running_episode_stats["count"] += (1 - ended) * (1 - masks)
        for k, v in self._extract_scalars_from_infos(infos).items():
            if k not in running_episode_stats:
                running_episode_stats[k] = torch.zeros_like(
                    running_episode_stats["count"]
            ...
            v = torch.tensor(v, dtype=torch.float, device=self.device)
            running_episode_stats[k] += (1 - ended) * (1 - masks) * v.unsqueeze(1)
        ...
        #     print(k, v.view(-1)) 
        # print('mask', masks.view(-1))
        # print('ended', ended.view(-1))
        # print('---------------------')
        rollouts.insert(
            action_elements,
            batch,
    ...
    def _collect_rollout_step_teacher(
        self,
        rollouts: ActionDictRolloutStorage,
        current_episode_reward: Tensor,
        running_episode_stats: Dict[str, Tensor],
        ...
        with torch.no_grad():
            ...
            actions_componets = step_observation['vln_oracle_waypoint_sensor']
            teacher_action_elements = {}
            teacher_action_elements['pano'] = actions_componets[..., 0].squeeze(0).unsqueeze(-1).long()
            teacher_action_elements['offset'] = actions_componets[..., 1].squeeze(0).unsqueeze(-1)
            teacher_action_elements['distance'] = actions_componets[..., 2].squeeze(0).unsqueeze(-1)

            # print('action shape', actions_componets.shape)
            thetas = actions_componets[..., 3].squeeze(0)
            actions = []
            for i, theta in enumerate(thetas):
                pano = teacher_action_elements['pano'][i]
                if pano == self.config.TASK_CONFIG.TASK.PANO_ROTATIONS:
                    actions.append({"action": "STOP"})
                else:
                    r = teacher_action_elements['distance'][i].item()
                    actions.append({
                        "action": {
                        "action": "GO_TOWARD_POINT",
                        "action_args": {
    ...
    def _collect_rollout_step_teacher_sample(
        self,
        rollouts: ActionDictRolloutStorage,
        current_episode_reward: Tensor,
        running_episode_stats: Dict[str, Tensor],
    ...
    def _update_agent(
        self, ppo_cfg: Config, rollouts: ActionDictRolloutStorage, teacher: bool 
    ) -> Tuple[float, float, float, float, float, float, float]:
        t_update_model = time.time()
        with torch.no_grad():
            last_observation = {
            ...
            last_observation['pre_e_t'] = rollouts.pre_e_t[:rollouts.step].to(self.device)
            last_observation['pre_q_t'] = rollouts.pre_q_t[:rollouts.step].to(self.device)
        ...
        if torch.isinf(next_value).any():
            print('value has inf', next_value)
            
        rollouts.compute_returns(
            next_value, ppo_cfg.use_gae, ppo_cfg.gamma, ppo_cfg.tau
        )
        if teacher:
            update_results = self.agent.update_teacher(rollouts)
        else:
            update_results = self.agent.update(rollouts)
        rollouts.after_update()
        return (time.time() - t_update_model, *update_results)
    ...
    def train(self) -> None:
        ...
        self.config.defrost()
        self.config.TORCH_GPU_ID = self.local_rank % num_gpu
        self.config.TASK_CONFIG.TASK.SPECTROGRAM_SENSOR.TORCH_GPU_ID = self.local_rank % num_gpu
        self.config.SIMULATOR_GPU_IDS = [self.local_rank % num_gpu]
        # Multiply by NUM_ENVIRONMENTS to ensure simulators get unique seeds
        self.config.TASK_CONFIG.SEED += (
            self.world_rank * self.config.NUM_ENVIRONMENTS
        )
        self.config.MODEL.TORCH_GPU_ID = self.config.TORCH_GPU_ID
        self.config.freeze()
        ...
        # task_idx = (self.world_rank // 4) % 4
        if 'instruction' in batch:
            task_idx = 0
        elif 'spectrogram' in batch:
            task_idx = 1
        elif 'imagegoal' in batch:
            task_idx = 2
        elif 'objectgoal' in batch:
            task_idx = 3
        else:
            assert False, 'no such task exist'
        ...
        with (
            ...
            for update in range(start_update, self.config.RL.NUM_UPDATES):
                ...
                for sensor in rollouts.observations:
                    rollouts.observations[sensor][0].copy_(batch[sensor].detach().cpu())
                ...
                for step in range(ppo_cfg.num_steps):
                    if not teacher_flag:
                        ...
                        (
                            ...
                            dones,
                            logging_predictions,
                        ) = self._collect_rollout_step_teacher(
                            rollouts, current_episode_reward, running_episode_stats,
                            ended.float()
                        )

                    done_flags = torch.tensor(
                        [[True] if done else [False] for done in dones], device=current_episode_reward.device
                    )
                    ended = ended | done_flags
                ...
                (
                    ...
                    offset_entropy,
                    distance_entropy,
                ) = self._update_agent(ppo_cfg, rollouts, False)
                ...
                stats_list = []
                for _ in task_name:
                    stats_list.append(torch.zeros_like(stats))
                stats_list = torch.stack(stats_list, 0)
                stats_list[task_idx] = stats
                # distrib.all_reduce(stats)
                distrib.all_reduce(stats_list)

                for j, task in enumerate(task_name):
                    for i, k in enumerate(stats_ordering):
                        window_episode_stats[j][k].append(stats_list[j,i].clone())
                ...
                for i, task in enumerate(task_name):
                    deltas = {
                        ...
                        for k, v in window_episode_stats[i].items()
                    }
                    deltas["count"] = max(deltas["count"], 1.0)
                    ...
                    if len(deltas) > len(ignore):
                        ...
                        # writer.add_scalars("metrics", metrics, count_steps)
                        for k, v in metrics.items():
                            writer.add_scalar(f"metrics/{task}/{k}", v, count_steps)
                ...
                if update % self.config.RL.CHECKPOINT_INTERVAL == 0:
                # if update % 5 == 0:
                    # print('save', update, f"ckpt.{count_checkpoints}-{self.world_rank}.pth")
                    requeue_stats = dict(
                        env_time=env_time,
                        pth_time=pth_time,
                        count_steps=count_steps,
    ...
    def _eval_checkpoint(
        ...
        count_steps = self._initialize_policy(
            config=config,
            load_from_ckpt=True,
            observation_space=self.obs_space,
            action_space=envs.action_spaces[0],
        ...
        # print('current step', count_steps)
        ...
        # obs_history = {
        #     "rgb_features": torch.zeros_like(batch["rgb_features"][:, 0]),
        #     "depth_features": torch.zeros_like(batch["depth_features"][:, 0]),
        # }
        obs_history = {}
        ...
        stats_episodes = {}
        infos = [{} for _ in range(envs.num_envs)]
        pre_e_t = torch.zeros(0, envs.num_envs, config.MODEL.STATE_ENCODER.hidden_size, device=self.device)
        pre_q_t = torch.zeros(0, envs.num_envs, config.MODEL.STATE_ENCODER.hidden_size, device=self.device)
        ...
        while envs.num_envs > 0 and len(stats_episodes) < num_eps:
            ...
            with torch.no_grad():
                batch_in = {k: v.detach().clone().unsqueeze(0) for k, v in batch.items()}
                batch_in['pre_e_t'] = pre_e_t
                batch_in['pre_q_t'] = pre_q_t
                deterministic = not self.config.EVAL.SAMPLE
                outputs = self.policy.act(
                    batch_in,
                    # rnn_states,
            ...
            for i in range(envs.num_envs):
                ...
                ep_id = current_episodes[i].episode_id + current_episodes[i].scene_id
                if not ep_id in stats_episodes:
                    stats_episodes[ep_id] = infos[i]
                    pbar.update()
                # observations[i] = envs.reset_at(i)[0]
            ...
            if np.all(dones) or pre_e_t.size()[0] == 40:
                for i in range(envs.num_envs):
                    ep_id = current_episodes[i].episode_id + current_episodes[i].scene_id
                    if not ep_id in stats_episodes:
                        stats_episodes[ep_id] = infos[i]
                ...
                next_episodes = envs.current_episodes()
                for i in range(envs.num_envs):
                    if next_episodes[i].episode_id + next_episodes[i].scene_id in stats_episodes:
                        envs_to_pause.append(i)
                pre_e_t = torch.zeros(0, envs.num_envs - len(envs_to_pause), config.MODEL.STATE_ENCODER.hidden_size, device=self.device)
                pre_q_t = torch.zeros(0, envs.num_envs - len(envs_to_pause), config.MODEL.STATE_ENCODER.hidden_size, device=self.device)
        ...
        for k, v in aggregated_stats.items():
            # logger.info(f"{k}: {v:.6f}")
            writer.add_scalar(f"eval/{split}/{k}", v, count_steps)


@baseline_registry.register_trainer(name="ddppo-waypoint-teacher")
class DDPPOWaypointTrainerTeacher(DDPPOWaypointTrainer):
    def _collect_rollout_step(
        self,
        rollouts: ActionDictRolloutStorage,
        current_episode_reward: Tensor,
        ...
        with torch.no_grad():
            ...
            # print('real components',action_elements)
            if np.random.rand() < 0:
                actions_componets = step_observation['vln_oracle_waypoint_sensor']
                action_elements = {}
                action_elements['pano'] = actions_componets[..., 0].unsqueeze(-1).long()
                action_elements['offset'] = actions_componets[..., 1].unsqueeze(-1)
                action_elements['distance'] = actions_componets[..., 2].unsqueeze(-1)
            
                thetas = actions_componets[..., 3]
                actions = []
                for i, theta in enumerate(thetas):
                    pano = action_elements['pano'][i]
                    if pano == self.config.TASK_CONFIG.TASK.PANO_ROTATIONS:
...
@baseline_registry.register_trainer(name="il")
class IL(DDPPOWaypointTrainer):
    def _initialize_policy(
        self,
        config: Config,
        load_from_ckpt: bool,
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            values, action_log_probs, _, recurrent_hidden_states = self.policy.evaluate_actions(step_observation,
                rollouts.recurrent_hidden_states[rollouts.step],
                step_prev_actions,
                rollouts.masks[rollouts.step],
                action_elements
                )
        ### after process teacher action
