...
class TruncatedNormal(nn.Module):
    ...
    def log_prob(self, value: Union[float, Tensor]) -> Tensor:
        ...
        if isinstance(value, Number):
            ...
            value = torch.clamp(value, self._smin.min(), self._smax.max())
            assert (value >= self._smin).all() and (
                value <= self._smax
            ).all(), msg + f"range is [{self._smin}, {self._smax}], value range is [{value.min()}, {value.max()}]"

        normal_prob_density = self.A * np.e ** (
            -0.5 * ((value - self._loc) / self._scale) ** 2
        )
        truncated_prob_density = normal_prob_density / self.Z + EPS


        if isinstance(truncated_prob_density, Number):
            return np.log(truncated_prob_density)
        else:
            if torch.isinf(truncated_prob_density.log()).any():
                print('density', truncated_prob_density)
...
class CustomFixedCategorical(torch.distributions.Categorical):
    ...
    def safe_prob(self, value: Tensor):
        if self._validate_args:
            self._validate_sample(value)
        value = value.long().unsqueeze(-1)
        # print('before', value.shape, self.probs.shape)
        value, pmf = torch.broadcast_tensors(value, self.probs)
        value = value[..., :1]
        probs = pmf.gather(-1, value).squeeze(-1) + EPS
        # print(value.shape, pmf.shape, probs.shape)
        return probs

    def log_probs(self, actions: Tensor) -> Tensor:
        probs = self.safe_prob(actions.squeeze(-1))
        return torch.log(probs).view(actions.size(0), -1).sum(-1).unsqueeze(-1)
        # return (
        #     super()
