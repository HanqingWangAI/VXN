...
class ContrastiveNet(WaypointPredictionNet):
    ...
    def __init__(self, observation_space: Space, model_config: Config) -> None:
        ...
        feat_size = self.rgb_encoder.output_shape[0]
        self.clenc = CLEnc(model_config, feat_size, False)
        self.clenc.load('data/checkpoints/cl_embedding/LAST_ITER_BK')
    ...
    def forward(
        ...
        rgb_size = rgb_obs.size()
        rgb_obs = rgb_obs.view((rgb_size[0] * rgb_size[1], *rgb_size[2:5])) # B x N x H x W x D
        rgb_embedding = self.rgb_encoder({"rgb": rgb_obs}) # B * N x D x H x W
        rgb_embedding = rgb_embedding.permute(0,2,3,1) # B * N x H x W x D
        rgb_embedding = self.clenc(rgb_embedding).permute(0, 3, 1, 2)  # B * N x D x H x W
        rgb_embedding = torch.flatten(
            rgb_embedding.view(*rgb_size[0:2], *rgb_embedding.shape[1:]), 3
        )
