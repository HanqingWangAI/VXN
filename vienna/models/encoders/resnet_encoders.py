...
from vienna.common.utils import single_frame_box_shape
...
class TorchVisionResNet(nn.Module):
    ...
    def extract_img(self, rgb):
        def normalize(imgs: Tensor) -> Tensor:
            ...
            if self.normalize_visual_inputs:
                ...
                return imgs
        embedding = self.cnn(normalize(rgb))
        return embedding
    ...
    def forward(self, observations: Observations) -> Tensor:
        ...
        if "rgb_features" in observations:
            ...
            resnet_output = self.extract_img(rgb_observations)
