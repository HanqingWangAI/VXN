    ...
    def compute_returns(
        ...
        if use_gae:
            ...
            for step in reversed(range(self.step)):
                ...
                assert not torch.isnan(self.returns[step]).any() and not torch.isinf(self.returns[step]).any(), (
                    f"Return is NaN.\nreward:\t{self.rewards.view(-1,bs)}"
                    f"\ngae:\t{gae}\ndelta:\t{delta}"
                    f"\nvalue_preds: {self.value_preds.view(-1, bs)}"
                    f"\nreturn: {self.returns.view(-1,bs)}"
                    f"\nmasks: {self.masks.view(-1, bs)}"
                )
        else:
            ...
            for step in reversed(range(self.step)):
                ...
                assert not torch.isnan(self.returns[step]).any() and not not torch.isinf(self.returns[step]).any(), (
...
class TFRolloutStorage(ActionDictRolloutStorage):
    def __init__(
        self,
        num_steps: int,
        num_envs: int,
        ...
        self.pre_e_t = torch.zeros(
            num_steps,
            num_envs,
            recurrent_hidden_state_size,
        )
        self.pre_q_t = torch.zeros(
            num_steps,
            num_envs,
            recurrent_hidden_state_size,
        )
        ...
        self.teacher_actions["pano"] = self.teacher_actions["pano"].long()
        super().__init__(num_steps, num_envs, observation_space, recurrent_hidden_state_size, num_recurrent_layers=num_recurrent_layers, continuous_offset=continuous_offset, continuous_distance=continuous_distance)

    def to(self, device):
        super().to(device)
        # self.pre_e_t = self.pre_e_t.to(device)
        # self.pre_q_t = self.pre_q_t.to(device)
        # for k in self.teacher_actions:
        #     self.teacher_actions[k] = self.teacher_actions[k].to(device)
    ...
    def insert(
        ...
        recurrent_hidden_states = recurrent_hidden_states.detach().cpu()
        action_log_probs = action_log_probs.detach().cpu()
        value_preds = value_preds.detach().cpu()
        rewards = rewards.detach().cpu()
        masks = masks.detach().cpu()
        e_t = e_t.detach().cpu()
        q_t = q_t.detach().cpu()
        for k in actions:
            actions[k] = actions[k].detach().cpu()
            teacher_actions[k] = teacher_actions[k].detach().cpu()
        for k in observations:
            observations[k] = observations[k].detach().cpu()

        self.pre_e_t[self.step].copy_(e_t[-1])
        self.pre_q_t[self.step].copy_(q_t[-1])
        for k in teacher_actions:
            self.teacher_actions[k][self.step].copy_(teacher_actions[k])
        super().insert(observations, recurrent_hidden_states, actions, action_log_probs, value_preds, rewards, masks)
    ...
    def recurrent_generator(
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                for k in self.actions:
                    actions_batch[k].append(self.teacher_actions[k][: self.step, ind])
                    prev_actions_batch[k].append(
                        self.prev_actions[k][: self.step, ind]
                    )
            ...
            for k in self.actions:
                actions_batch[k] = self._flatten_helper(T, N, actions_batch[k]).to(self.device)
                prev_actions_batch[k] = self._flatten_helper(
                    T, N, prev_actions_batch[k]
                ).to(self.device)
            ...
            yield (
                observations_batch,
                recurrent_hidden_states_batch.to(self.device),
                actions_batch,
                prev_actions_batch,
                value_preds_batch.to(self.device),
                return_batch.to(self.device),
                masks_batch.to(self.device),
                old_action_log_probs_batch.to(self.device),
                adv_targ.to(self.device),
            )
